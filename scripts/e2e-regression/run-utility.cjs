...
/**
 * utility-01: Welcome page renders (consent or onboarding — intentional for first login)
 * The page renders without a temp_token. React shows consent/onboarding content.
 * Leptos must do the same — redirecting to /login is a regression.
 */
async function testWelcome(server) {
  ...
  try {
    ...
    // Must NOT redirect to /login — page should render content
    const url = page.url();
    assertions.push(!url.includes('/login')
      ? result(true, `Welcome page renders at: ${url}`)
      : result(false, `Welcome page redirected to /login — broken (React shows consent/onboarding content here): ${url}`)
    );

    // Should have substantive content
    const body = await page.textContent('body');
    assertions.push(body && body.length > 200
      ? result(true, 'Page has content')
      : result(false, `Page body too short (${body ? body.length : 0} chars) — likely error/blank state`)
    );
...
const FLOWS = [
  { id: 'utility-01', name: 'Welcome page renders consent/onboarding content', fn: testWelcome, requiresAuth: false },
...
function classify(react, leptos) {
  ...
  // Behaviors differ and Leptos passes React fails — could be wrong test OR divergence.
  // Never assume it's an improvement. Treat as BEHAVIORAL_DIFF (failure requiring review).
