...
/**
 * OAuth callback handler for Microsoft LOGIN flow (not datasource link flow)
...
export default function MicrosoftLoginCallback() {
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      ...
      if (error) {
        setStatus('error');
        setMessage(`Microsoft OAuth error: ${errorDescription || error}`);
        if (isInPopup) {
          setTimeout(() => {
            window.opener.postMessage({
              type: 'MICROSOFT_OAUTH_ERROR',
              error: `Microsoft OAuth error: ${errorDescription || error}`
      ...
      try {
        ...
        if (data.status === 'pending_terms') {
          ...
          if (isInPopup) {
            setTimeout(() => {
              window.opener.postMessage({
                type: 'MICROSOFT_OAUTH_TERMS_REQUIRED',
                data: data
              }, window.location.origin);
              window.close();
            }, 1000);
        ...
        const errorMsg = error.response?.data?.detail || error.message || 'Failed to sign in with Microsoft';
        setStatus('error');
        setMessage(errorMsg);
