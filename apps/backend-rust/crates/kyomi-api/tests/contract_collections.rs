...
//! Contract tests for collection REST endpoints.
//!
//! Verifies HTTP-level contract (status codes, auth enforcement, response format)
//! for all endpoints under `/api/v1/collections`.
...
async fn create_collection_returns_401_without_auth() {
    ...
    let resp = client()
        .post(format!("{base}/api/v1/collections"))
        .header("origin", "http://localhost:5173")
        .header("content-type", "application/json")
        .body(r#"{"name": "Test Collection"}"#)
        .send()
        .await
        .unwrap();
...
async fn list_collections_returns_401_without_auth() {
    let base = base_url().await;
    let resp = client()
        .get(format!("{base}/api/v1/collections"))
        .header("origin", "http://localhost:5173")
        .send()
        .await
        .unwrap();
...
async fn get_collection_returns_401_without_auth() {
    let base = base_url().await;
    let resp = client()
        .get(format!("{base}/api/v1/collections/col-test123"))
        .header("origin", "http://localhost:5173")
        .send()
        .await
        .unwrap();
...
async fn update_collection_returns_401_without_auth() {
    let base = base_url().await;
    let resp = client()
        .patch(format!("{base}/api/v1/collections/col-test123"))
        .header("origin", "http://localhost:5173")
        .header("content-type", "application/json")
        .body(r#"{"name": "Updated"}"#)
        .send()
...
async fn delete_collection_returns_401_without_auth() {
    let base = base_url().await;
    let resp = client()
        .delete(format!("{base}/api/v1/collections/col-test123"))
        .header("origin", "http://localhost:5173")
        .send()
        .await
        .unwrap();
...
async fn add_dashboard_to_collection_returns_401_without_auth() {
    ...
    let resp = client()
        .post(format!(
            "{base}/api/v1/collections/col-test123/dashboards"
        ))
        .header("origin", "http://localhost:5173")
        .header("content-type", "application/json")
        .body(r#"{"dashboard_id": "dash-test123"}"#)
        .send()
        .await
        .unwrap();
...
async fn remove_dashboard_from_collection_returns_401_without_auth() {
    ...
    let resp = client()
        .delete(format!(
            "{base}/api/v1/collections/col-test123/dashboards/dash-test123"
        ))
        .header("origin", "http://localhost:5173")
        .send()
        .await
...
// ---- CORS on collection endpoints ----

#[tokio::test]
async fn collection_endpoints_have_cors_headers() {
    let base = base_url().await;
    let resp = client()
        .get(format!("{base}/api/v1/collections"))
        .header("origin", "http://localhost:5173")
    ...
    assert_eq!(
        ...
        "CORS should be present on collection endpoints"
    );
}
...
async fn collection_error_responses_have_detail_field() {
    ...
    let endpoints = vec![
        ...
        ("GET", "/api/v1/collections/col-nonexistent"),
        ("DELETE", "/api/v1/collections/col-nonexistent"),
    ];
