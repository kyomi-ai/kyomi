...
async fn setup_auth_context(suffix: &str) -> Option<AuthContext> {
    ...
    let email = format!("slack-test-{suffix}@contract-test.local");

    cleanup_test_user(&db, &email).await;

    let user = kyomi_auth::user_service::create_user(&db, &email, Some("Slack Test User"), true)
        .await
    ...
    extra.insert("name".to_string(), json!("Slack Test User"));
...
async fn setup_team_auth_context(suffix: &str) -> Option<AuthContext> {
    ...
    // Upgrade the workspace to Team tier
