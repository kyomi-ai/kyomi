...
-- Add doc_type to collections so they are scoped to either dashboards or
-- knowledge. Mirrors Postgres migration 20260410000001_collection_doc_type.
-- Previously collections were type-agnostic and filtering relied on JOINing
-- to the dashboards table, which caused empty collections to disappear.

ALTER TABLE collections ADD COLUMN doc_type TEXT NOT NULL DEFAULT 'dashboard';
