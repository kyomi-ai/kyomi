...
-- private.
--
-- 00024_add_sync_log_visibility.sql backfilled every historical row for
-- these three types to is_workspace_visible = 0 unconditionally (see its
-- second and third UPDATEs). That is wrong in the *opposite* direction from
-- KYO-237's watch bug: visibility for these types is computed per row at
-- write time --
...
-- is_workspace_visible = 0 forever, while the entity's *new* rows are
-- correctly written 1. sync_log_service::get_entries_since filters on
...
-- sets it), that signature does not exist here -- 00024's backfill extracted
-- owner_user_id correctly for dashboard/knowledge/chat_session
-- (json_extract(data, '$.user_id') / json_extract(data,
-- '$.created_by.user_id')), so a NULL-owner guard cannot distinguish a
-- backfilled row from a correctly-written one.
--
-- Instead this migration guards on `is_workspace_visible = 0` and
...
--     (chat_service.rs:704-708) -- not `json_extract(data, '$.shared')` off
--     the row's own payload. The payload reflects the share state *at the
--     time that row was written*, which can go stale (a session can be
--     shared, then unshared, then shared again); the live predicate can
--     only ever be the session's *current* shared column. Recomputing from
--     the payload could mark a row workspace-visible whose current state is
--     private -- the exact leak this migration exists to avoid introducing
--     on the other type.
--
...
--      is_workspace_visible, and only 0 -> 1; owner_user_id is never
...
--      not a bug -- the alternative (leaving it 0 forever) is just a
...
-- (FOREIGN KEY ... ON DELETE CASCADE), and a deleted chat_sessions row
-- simply no longer exists to join against. The EXISTS below finds no match,
-- is_workspace_visible stays 0, and a non-owner who had the entity cached
-- from before this fix never learns it was deleted -- it lingers in their
-- local cache indefinitely. This is pre-existing behaviour, symmetric
-- between dashboards and chat sessions, and predates this migration rather
-- than being introduced by it.
UPDATE sync_log SET is_workspace_visible = 1
 WHERE entity_type IN ('dashboard', 'knowledge')
   AND is_workspace_visible = 0
   AND EXISTS (
