...
async fn setup_auth_context(suffix: &str) -> Option<AuthContext> {
    ...
    let email = format!("bq-test-{suffix}@contract-test.local");

    // Clean up any leftover test data from a previous run
    cleanup_test_user(&db, &email).await;

    // Create a verified user
    let user = kyomi_auth::user_service::create_user(&db, &email, Some("BQ Test User"), true)
        .await
    ...
    extra.insert("name".to_string(), json!("BQ Test User"));
...
async fn cleanup_test_user(db: &kyomi_core::DbPool, email: &str) {
    ...
    if let Some(uid) = user_id {
        ...
        for ws_id in &workspace_ids {
            // Delete search embeddings for datasources in this workspace
