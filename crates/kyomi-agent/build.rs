...
//! Build script for `kyomi-agent`.
//!
//! `src/tools/watch.rs` expands `sqlx::migrate!("../../apps/server/migrations-sqlite")`
//! at compile time. That macro embeds the migration files that exist *when
...
fn main() {
    ...
    rerun_if_migrations_dir_changed("kyomi-agent", &migrations_sqlite);
}
