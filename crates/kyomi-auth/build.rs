...
//! Build script for `kyomi-auth`.
//!
//! This crate's test modules expand `sqlx::migrate!("../../apps/server/migrations-sqlite")`
//! at 17 call sites. That macro embeds the migration files that exist *when
//! the macro expands* by generating `include_bytes!` references to each one
//! — so editing an existing `.sql` file marks the crate dirty and triggers a
...
//! Every test in this crate that reaches a DB then silently runs against the
...
fn main() {
    ...
    rerun_if_migrations_dir_changed("kyomi-auth", &migrations_sqlite);
}
