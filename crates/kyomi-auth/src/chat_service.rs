...
/// - [`add_message`] / [`update_session`] are only ever invoked with a
///   `session_id` that is either freshly generated by the callee (a new
///   session/watch-execution session it just created) or was resolved via
///   [`get_session_info`]'s owner-or-shared check earlier in the same
///   request (see `prepare_chat_dispatch`, `copilot_service::prepare_copilot_message`).
