...
//!
//! These live in `kyomi-auth` (shared service layer) so that both the REST
//! server routes and Leptos server functions can call them without circular
//! dependencies.
...
/// Gracefully handles missing config (logs and continues, never errors).
pub async fn notify_signup(
    ...
    send_signup_email(support_email, email, name, user_id).await;
}
...
async fn send_signup_email(support_email: &str, email: &str, name: &str, user_id: &str) {
