...
impl BigQueryPublicIndexer {
    ...
    pub async fn get_public_table_count(db: &DbPool) -> i64 {
        crate::datasource_service::count_tables_for_workspace(db, PUBLIC_DATA_WORKSPACE_ID, None)
            .await
            .unwrap_or_else(|e| {
                warn!(error = %e, "failed to count cached public-dataset tables; defaulting to 0");
                0
            })
    }
}
