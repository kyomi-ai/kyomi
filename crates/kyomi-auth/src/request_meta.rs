...
//! Request-metadata extraction — client IP and device info.
//!
//! **Canonical.** These two functions used to be reimplemented once per
//! caller (three copies of `extract_device_info`, a fourth copy of
//! `extract_client_ip` in `kyomi-ui`) because the originals lived in
//! `apps/server`, a binary crate that `kyomi-ui` cannot depend on. The
//! copies diverged: the `kyomi-ui` copy dropped the `peer_addr` TCP-peer
//! fallback entirely, silently keying every rate-limit bucket to
//! `"unknown"` in local dev (behind nginx in production, `X-Real-IP` is
//! always set, so this was invisible there). See KYO-194.
//!
//! Both functions now live here so every caller — REST route handlers in
//! `apps/server` and Leptos server functions in `kyomi-ui` — shares one
//! implementation. Callers that cannot supply a TCP peer address (most
//! Leptos server fns, today) pass `None` explicitly, so the limitation is
//! visible at the call site instead of baked into a separate copy.
...
///
/// Used by login and signup endpoints that need to record device info for
/// sessions. `oauth_client_id` is always `None` here — callers that need to
/// attach an OAuth client id (MCP token issuance) set it on the returned
/// `DeviceInfo` themselves rather than reimplementing extraction.
...
/// ```
/// # use axum::http::HeaderMap;
/// # use kyomi_auth::request_meta::extract_client_ip;
