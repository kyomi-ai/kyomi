...
pub fn SaveDashboardModal(
    ...
    // ── Unified save Action ──────────────────────────────────────────────
    // Enum-like input: (is_create, title_or_empty, dashboard_id_or_empty, chart_yaml)
    // When is_create=true: creates a new dashboard with the given title.
    // When is_create=false: fetches dashboard_id, appends chart_yaml, and saves.
    type SaveModalInput = (bool, String, String, String); // (is_create, title, dashboard_id, chart_yaml)

    let save_action = Action::new(|input: &SaveModalInput| {
        let (is_create, title, dashboard_id, chart_yaml) = input.clone();
        async move {
            if is_create {
                create_dashboard(title, Some(chart_yaml)).await.map(|id| (id, true))
            } else {
    ...
    let handle_save = Callback::new(move |()| {
        ...
        if creating {
            ...
            set_error.set(None);
            save_action.dispatch((true, title.trim().to_string(), String::new(), chart_yaml));
        } else {
            ...
            let chart_yaml = chart_yaml_signal.get_untracked();
            set_error.set(None);
            save_action.dispatch((false, String::new(), dashboard_id, chart_yaml));
        }
    });
    ...
    let footer_view: ChildrenFn = Arc::new(move || {
        ...
        view! {
            <button
                ...
                disabled=save_action.pending().get()
            >
                "Cancel"
            </button>
            <button
