...
pub fn Layout(children: ChildrenFn) -> impl IntoView {
    ...
    // both the "Send Feedback" nav item and the single `FeedbackModal`
    // instance.
...
fn Sidebar(
    ...
    /// Owned by `Layout`, not `Sidebar` — see `Layout`'s body.
    ...
    // `feedback_open` is owned by `Layout` (passed in as a prop), not
    // created here — see `Layout`'s own body.
