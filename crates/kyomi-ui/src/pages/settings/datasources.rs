...
mod tests {
    ...
    enum CreateNextRoute {
        /// The shared "Test & Discover" button (~line 3763): visible for
        /// every type/mode except the ones the other four routes below
        /// cover, and its `on:click` runs `do_test_and_discover()`, whose
        /// `test_action` Effect writes `test_result.success = true` on a
        /// successful probe.
        ...
        /// button (~line 4959), wired through `on_validate` to the same
        /// `do_test_and_discover` every other provider's generic button
        /// calls.
