...
pub fn UnsubscribePage() -> impl IntoView {
    ...
    let unsubscribe_action = Action::new(|email: &String| {
        let email = email.clone();
        async move { unsubscribe_email(email).await }
    });

    // React to the action result: set success or error state.
    Effect::new(move |_| {
        if let Some(result) = unsubscribe_action.value().get() {
            match result {
    ...
    // Derived: disable submit when email is empty or action is pending.
    let submit_disabled = Memo::new(move |_| {
        unsubscribe_action.pending().get() || email.get().trim().is_empty()
    });
    ...
    let on_submit = move |ev: web_sys::SubmitEvent| {
        ...
        unsubscribe_action.dispatch(email_val);
    };
                        ...
                        fallback=move || {
                            view! {
                                <form on:submit=on_submit class="space-y-5">
                                    ...
                                    >
                                        <Show
                                            when=move || unsubscribe_action.pending().get()
                                            fallback=|| {
                                                view! { "Unsubscribe" }
                                            }
                                        >
