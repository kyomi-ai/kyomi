...
pub fn WelcomePage() -> impl IntoView {
    ...
    // ── Accept terms Action ──────────────────────────────────────────────
    let accept_action = Action::new(|(token, consent): &(String, bool)| {
        let token = token.clone();
        let consent = *consent;
        async move { accept_terms(token, consent).await }
    });

    // React to the action result: navigate on success, set error on failure.
    let nav_result = navigate.clone();
    Effect::new(move |_| {
        if let Some(result) = accept_action.value().get() {
            match result {
                Ok(AcceptTermsResult::Success) => {
                    nav_result.clone()("/onboarding", Default::default());
                }
    ...
    let on_submit = move |_| {
        if !agreed.get_untracked() || accept_action.pending().get_untracked() {
            return;
        }
        set_error.set(None);
        let token = temp_token.get_untracked();
        let consent = marketing_consent.get_untracked();
        accept_action.dispatch((token, consent));
    };

    // ── Derived: button disabled ─────────────────────────────────────────
    let button_disabled = move || !agreed.get() || accept_action.pending().get();
