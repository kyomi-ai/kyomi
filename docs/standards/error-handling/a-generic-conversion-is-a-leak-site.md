...
BigQuery's `list_projects()` override returns `Provider`/`Internal`; the trait's *default*
`list_projects()` returns the tagged `NotSupported(…)`. Correct today, by a fact about which
impl is selected rather than about the type.
